...
public class SequenceCommand
{
    ...
    ///     Command parameters.
    /// </summary>
    public static Dictionary<SequenceCommands, SequenceCommandParameter> CommandParameters = new()
    {
    ...
    public void Read(FileReader r, SequencePlatform p, ParameterMode parameterMode = ParameterMode.Normal)
    {
        ...
        //Note.
        if (identifier < 0x80) CommandType = SequenceCommands.Note;
        ...
        switch (CommandParameters[CommandType])
        {
            ...
            case SequenceCommandParameter n when (int)CommandType < 0x80:
                ...
                {
                    Note = (Notes)identifier, Velocity = r.ReadByte(), Length = VariableLength.ReadVariableLength(r, 4)
                };
                break;
            ...
            case SequenceCommandParameter.VariableLength:
                if (parameterMode == ParameterMode.NoParameter) return;
    ...
    public void Write(FileWriter w, SequencePlatform p, ParameterMode parameterMode = ParameterMode.Normal)
    {
        ...
        switch (CommandParameters[CommandType])
        {
            ...
            //If.
            case SequenceCommandParameter.If:
                (Parameter as SequenceCommand).Write(w, p);
                break;

            //Time.
            case SequenceCommandParameter.Time:
                (Parameter as TimeParameter).Command.Write(w, p);
                w.Write((Parameter as TimeParameter).Value);
                break;

            //Time random.
            case SequenceCommandParameter.TimeRandom:
                (Parameter as RandomParameter).Command.Write(w, p);
                w.Write((Parameter as RandomParameter).Min);
                w.Write((Parameter as RandomParameter).Max);
                break;
            ...
            //Time variable.
            case SequenceCommandParameter.TimeVariable:
                (Parameter as VariableParameter).Command.Write(w, p);
                w.Write((Parameter as VariableParameter).Variable);
                break;
    ...
    public Tuple<string, List<string>> ToString(bool noParameters)
    {
        ...
        switch (CommandParameters[CommandType])
        {
            ...
            case SequenceCommandParameter n when (int)CommandType < 0x80:
                ...
                if (!noParameters) data.Add((Parameter as NoteParameter).Length.ToString());
                break;

            //Open track.
            case SequenceCommandParameter.OpenTrack:
                data.Add((Parameter as OpenTrackParameter).TrackNumber.ToString());
                if (!noParameters) data.Add((Parameter as OpenTrackParameter).Label);
                break;

            //Variable length.
            case SequenceCommandParameter.VariableLength:
                if (!noParameters) data.Add(Parameter.ToString());
                break;

            //U24.
            case SequenceCommandParameter.U24:
                if (!noParameters) data.Add((Parameter as UInt24Parameter).Label);
                break;
            ...
            case SequenceCommandParameter.U8S16:
                ...
                if (!noParameters) data.Add((Parameter as U8S16Parameter).S16.ToString());
                break;
    ...
    public void FromString(string s, Dictionary<string, int> publicLabels, Dictionary<string, int> privateLabels)
    {
        ...
        //Footer flags.
        if (c.Contains("_if_") || c.Contains("_if ") || c.EndsWith("_if")) isIf = true;
        ...
        if (c.Contains("_tv_") || c.Contains("_tv ") || c.EndsWith("_tv"))
            ...
            isTimeRandom = true;
        else if (c.Contains("_t_") || c.Contains("_t ") || c.EndsWith("_t")) isTime = true;

        //Header flags.
        if (c.Contains("_v_") || c.Contains("_v ") || c.EndsWith("_v"))
            isVariable = true;
        else if (c.Contains("_r_") || c.Contains("_r ") || c.EndsWith("_r")) isRandom = true;

        //Bool data.
        var boolParam = false;
        if (c.Contains("on")) boolParam = true;
        ...
        var noLastParameter = isRandom || isVariable;
        ...
        switch (CommandParameters[b])
        {
            ...
            case SequenceCommandParameter.NoteParam:
                ...
                if (!noLastParameter)
                    Parameter = new NoteParameter
                    {
                        ...
                        Velocity = (byte)ParseData(data[dataPtr++], publicLabels, privateLabels),
            ...
            case SequenceCommandParameter.OpenTrack:
                if (!noLastParameter)
                    ...
                    {
                        TrackNumber = (byte)ParseData(data[dataPtr++], publicLabels, privateLabels),
                ...
                break;
            case SequenceCommandParameter.S16:
                if (!noLastParameter) Parameter = (short)ParseData(data[dataPtr++], publicLabels, privateLabels);
                break;
            case SequenceCommandParameter.U16:
                if (!noLastParameter) Parameter = (ushort)ParseData(data[dataPtr++], publicLabels, privateLabels);
                break;
            case SequenceCommandParameter.U24:
                if (!noLastParameter)
                ...
                break;
            case SequenceCommandParameter.U8:
                if (!noLastParameter) Parameter = (byte)ParseData(data[dataPtr++], publicLabels, privateLabels);
                break;
            case SequenceCommandParameter.S8:
                if (!noLastParameter) Parameter = (sbyte)ParseData(data[dataPtr++], publicLabels, privateLabels);
                break;
            case SequenceCommandParameter.U8S16:
                if (!noLastParameter)
                ...
                break;
            case SequenceCommandParameter.VariableLength:
                if (!noLastParameter) Parameter = (uint)ParseData(data[dataPtr++], publicLabels, privateLabels);
                break;
        }
        ...
        if (isRandom)
        {
            ...
            {
                Command = Duplicate(), Min = (short)ParseData(data[dataPtr++], publicLabels, privateLabels),
        ...
        else if (isVariable)
        {
            Parameter = new VariableParameter
                { Command = Duplicate(), Variable = (byte)ParseData(data[dataPtr++], publicLabels, privateLabels) };
        ...
        if (isTime)
        {
            Parameter = new TimeParameter
                { Command = Duplicate(), Value = (short)ParseData(data[dataPtr++], publicLabels, privateLabels) };
    ...
    {
        //Labels.
        if (publicLabels.ContainsKey(data)) return publicLabels[data];
        if (privateLabels.ContainsKey(data)) return privateLabels[data];
        ...
        switch (CommandParameters[seq.CommandType])
        {
            ...
            case SequenceCommandParameter.NoteParam:
                try
                {
                    ...
                    {
                        Note = (Parameter as NoteParameter).Note, Length = (Parameter as NoteParameter).Length,
                ...
                {
                    seq.Parameter = new NoteParameter
                        { Note = (Parameter as NoteParameter).Note, Velocity = (Parameter as NoteParameter).Velocity };
                }
            ...
            case SequenceCommandParameter.Random:
                seq.Parameter = new RandomParameter
                {
                    ...
                    Min = (Parameter as RandomParameter).Min, Max = (Parameter as RandomParameter).Max
                };
                break;
            case SequenceCommandParameter.S16:
                try
            ...
            case SequenceCommandParameter.U24:
                try
                {
                    seq.Parameter = new UInt24Parameter
                    {
                        ...
                        Label = (Parameter as UInt24Parameter).Label, Offset = (Parameter as UInt24Parameter).Offset
                    };
                }
                catch
                {
            ...
            case SequenceCommandParameter.U8S16:
                try
                {
                    seq.Parameter = new U8S16Parameter
                        { U8 = (Parameter as U8S16Parameter).U8, S16 = (Parameter as U8S16Parameter).S16 };
                }
                catch
                {
                    seq.Parameter = new U8S16Parameter { U8 = (Parameter as U8S16Parameter).U8 };
                }
