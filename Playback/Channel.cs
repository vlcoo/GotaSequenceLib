...
public class Channel
{
    ...
    public int
        Velocity; // The SEQ Player treats 0 as the 100% amplitude value and -92544 (-723*128) as the 0% amplitude value. The starting ampltitude is 0% (-92544).
    ...
    public void Stop()
    {
        if (Owner != null) Owner.Channels.Remove(this);
        Owner = null;
        Volume = 0;
    }
    ...
    public int SweepMain()
    {
        ...
        {
            var sweep = (int)(Math.BigMul(SweepPitch, SweepLength - SweepCounter) / SweepLength);
            if (AutoSweep) SweepCounter++;
            return sweep;
        }
    ...
    public void StepEnvelope()
    {
        switch (State)
        {
            case EnvelopeState.Attack:
            {
                ...
                if (Velocity == 0) State = EnvelopeState.Decay;
                break;
            }
            ...
            {
                Velocity -= _release;
                if (Velocity < -92544) Velocity = -92544;
                break;
            }
        }
    }
    ...
    public void EmulateProcess()
    {
        if (Timer != 0)
        {
            var numSamples = (_pos + 0x100) / Timer;
            ...
            for (var i = 0; i < numSamples; i++)
            {
                ...
                switch (Type)
                {
                    case InstrumentType.PCM:
                    {
                        if (_wave != null)
                        {
                            ...
                            else if (_wave.Audio.Channels[0][0] as PCM8 != null)
                            {
                                ...
                                {
                                    lSample =
                                        (short)((((byte[])(_wave.Audio.Channels[0][0] as PCM8).RawData())[_waveSample] -
                                                 128) << 8);
                                    rSample =
                                        (short)((((byte[])(_wave.Audio.Channels[1][0] as PCM8).RawData())
                                            [_waveSample++] - 128) << 8);
                                }
                                else
                                {
                                    samp = (short)((((byte[])(_wave.Audio.Channels[0][0] as PCM8).RawData())[
                                        _waveSample++] - 128) << 8);
                                }
                            }
                            else
                    ...
                    case InstrumentType.PSG:
                    {
                        ...
                        _psgCounter++;
                        if (_psgCounter >= 8) _psgCounter = 0;
                        break;
                    }
